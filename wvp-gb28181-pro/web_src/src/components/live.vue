<template>
  <div id="devicePosition" style="width:100vw; height: 91vh">
    <el-container v-loading="loading" style="height: 85vh;" element-loading-text="拼命加载中">
      <el-aside width="300" style="background-color: #ffffff;">
        ...
        <el-main style="padding: 0;">
          <div style="width: 99%;height: 85vh;display: flex;flex-wrap: wrap;background-color: #000;">
            <div v-for="i in spilt" :key="i" class="play-box"
                 :style="playerIdx === (i - 1) ? activeStyle : hiddenStyle"
                 >
              <div v-if="!videoUrl[i-1]" style="color: #ffffff;font-size: 30px;font-weight: bold;">{{ i }}</div>
              <player :playerIdx="playerIdx" ref="player" v-else :videoUrl="videoUrl[i-1]" fluent autoplay @screenshot="shot" @updateValue="updateParentValue"
                      @destroy="destroy"/>
            </div>
          </div>
        </el-main>
...
export default {
  ...
  mounted() {
this.setPlayUrl("https://cdn.theguardian.tv/webM/2015/07/20/150716YesMen_synd_768k_vp8.webm", 0);
  },
  created() {
  ...
  updateParentValue(newValue) {
    ...
    sendDevicePush: function (itemData) {
      ...
      this.$axios({
        ...
        if (res.data.code === 0 && res.data.data) {
          ...
          console.log("开始setPlayUrl"+videoUrl + "设备id" + deviceId)
          that.setPlayUrl(videoUrl, idxTmp,deviceId);
        } else {
    ...
    setPlayUrl(url, idx,deviceId) {
      console.log("进入setPlayUrl")
      this.$set(this.videoUrl, idx, url)
      let _this = this
      setTimeout(() => {
        window.localStorage.setItem('videoUrl', JSON.stringify(_this.videoUrl))
      }, 100)
		  this.sendVideoUrlToBackend(url,deviceId);
    },
	 sendVideoUrlToBackend(url,deviceId) {
    console.log("进入sendVideoUrlToBackend"+url+deviceId)
    // 假设你已经设置好了 axios 用于发送 HTTP 请求
    this.$axios({
      method: 'post',
      url: '/api/device/query/stream', // 替换成你的后端接口地址
      params: {
        videoUrl: url,
		    deviceId: deviceId
      }
    ...
    .then(response => {
      // 成功发送后的处理逻辑
      console.log(`成功发送播放器 ${idx + 1} 的视频 URL`, response.data);
    })
    .catch(error => {
      // 发送失败后的处理逻辑
      console.error(`发送播放器 ${idx + 1} 的视频 URL 失败`, error);
    });
  },
