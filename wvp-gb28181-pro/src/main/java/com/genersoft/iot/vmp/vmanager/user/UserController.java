...
public class UserController {
    ...
    public String sendSms() {
        smsService.sendSmsToUsers("34020000001320000001");
        return "短信发送任务已启动";
    }
    ...
    public LoginUser login_app(HttpServletRequest request, HttpServletResponse response, @RequestParam String username, @RequestParam String password){
        LoginUser user;
        try {
    ...
    public LoginUser login(HttpServletRequest request, HttpServletResponse response, @RequestParam String username, @RequestParam String password){
        // 从数据库中获取用户角色（您可以使用服务类或存储库）
        int roleId = userService.getUser(username,password).getRole().getId(); // 获取用户角色ID的示例方法
        ...
        {
            //其余用户无权登录
            throw new ControllerException(ErrorCode.ERROR100.getCode(), "无权限登录");
        }
    }
    ...
    @Parameter(name = "name", description = "添加角色用户的角色名", required = true)
    @Parameter(name = "email", description = "添加角色用户的邮箱", required = true)
    @Parameter(name = "phoneNumber", description = "添加角色用户的手机号", required = true)
    public void add(@RequestParam String username,
                                                 @RequestParam String password,
                    @RequestParam String name,
                    @RequestParam String email,
                    @RequestParam String phoneNumber){
        if (ObjectUtils.isEmpty(username) || ObjectUtils.isEmpty(password) || ObjectUtils.isEmpty(name)) {
            throw new ControllerException(ErrorCode.ERROR400.getCode(), "参数不可为空");
        }
        // 获取当前登录用户id
        int currenRoleId = SecurityUtils.getUserInfo().getRole().getId();
        if (currenRoleId != 1 && currenRoleId != 2) {
            // 只用角色id为1才可以删除和添加用户
        ...
        // 获取name在wvp_user_role表中的role_id
        Integer roleId = roleService.getRoleIdByName(name);
        if (roleId == null) {
    ...
    @Operation(summary = "web注册用户", security = @SecurityRequirement(name = JwtUtils.HEADER))
    @Parameter(name = "username", description = "用户名", required = true)
    @Parameter(name = "password", description = "密码（未md5加密的密码）", required = true)
    ...
    public void register(@RequestParam String username,
                            ...
                            @RequestParam String email,
                             @RequestParam String phoneNumber){
        if (ObjectUtils.isEmpty(username) || ObjectUtils.isEmpty(password) || ObjectUtils.isEmpty(email) || ObjectUtils.isEmpty(phoneNumber)) {
            throw new ControllerException(ErrorCode.ERROR400.getCode(), "参数不可为空");
        }
        User user = new User();
        user.setUsername(username);
        ...
        user.setCreateTime(DateUtil.getNow());
        user.setUpdateTime(DateUtil.getNow());
        int addResult = userService.addRegUser(user);
        if (addResult <= 0) {
            throw new ControllerException(ErrorCode.ERROR100.getCode(), "添加用户失败");
        }
    }
    ...
    @Operation(summary = "小程序注册用户")
    @Parameter(name = "username", description = "用户名", required = true)
    @Parameter(name = "password", description = "密码（未md5加密的密码）", required = true)
    @Parameter(name = "phoneNumber", description = "手机号", required = true)
    public void register_app(@RequestParam String username,
                       @RequestParam String password,
                       @RequestParam String phoneNumber){
        if (ObjectUtils.isEmpty(username) || ObjectUtils.isEmpty(password) || ObjectUtils.isEmpty(phoneNumber)) {
            throw new ControllerException(ErrorCode.ERROR400.getCode(), "参数不可为空");
        }
        User user = new User();
        user.setUsername(username);
    ...
    @DeleteMapping("/deny")
    @Operation(summary = "删除申请表用户", security = @SecurityRequirement(name = JwtUtils.HEADER))
    @Parameter(name = "id", description = "用户Id", required = true)
    public void deleteRegUser(@RequestParam Integer id){
        // 获取当前登录用户id
        int currenRoleId = SecurityUtils.getUserInfo().getRole().getId();
        if (currenRoleId != 1 && currenRoleId != 2) {
            // 只用角色id为0才可以删除和添加用户
        ...
        int deleteResult = userService.deleteRegUser(id);
        if (deleteResult <= 0) {
            throw new ControllerException(ErrorCode.ERROR100);
        }
    }
    ...
    public PageInfo<User> all(int page, int count){
        // 获取当前登录用户id
        return userService.getAllUsers(page, count);
    }

    @GetMapping("/all/regUsers")
    @Operation(summary = "查询申请注册用户", security = @SecurityRequirement(name = JwtUtils.HEADER))
    public List<User> allRegUsers(){
        // 获取当前登录用户id
        return userService.getAllRegUsers();
    }
    ...
    @PostMapping("/agree")
    @Operation(summary = "同意用户", security = @SecurityRequirement(name = JwtUtils.HEADER))
    @Parameter(name = "id", description = "用户ID", required = true)
    @Parameter(name = "name", description = "角色名", required = true)
    public void agree(@RequestParam Integer id, @RequestParam String name) {
        ...
        System.out.println("/api的id是"+id);
        // 查找wvp_reg_user表中的用户
        User regUser = userService.getUserById(id);
        if (regUser == null) {
        ...
        smsService.sendSmsToRegisterUsers(regUser.getPhoneNumber(),regUser.getUsername());
        ...
        if(regUser.getPhoneNumber() != null)
        {
        ...
        user.setPushKey(DigestUtils.md5DigestAsHex((System.currentTimeMillis() + regUser.getUsername()).getBytes()));
...
//    @Parameter(name = "username", description = "用户ID", required = true)
//    @Parameter(name = "phoneNumber", description = "角色名")
//    @Parameter(name = "email", description = "用户ID", required = true)
//    public void updateUser(@ModelAttribute User user) {
//        System.out.println("user:"+user.getRole());
//        user.setUpdateTime(DateUtil.getNow());
//        int updateResult = userService.updateUsers(user);
//        if (updateResult <= 0) {
//            throw new ControllerException(ErrorCode.ERROR100);
//        }
//
